...
def datagen(
    ...
    # JIT OPTIMIZATION: Check if we need to pad to avoid recompilation
    if len(latent_batch) > 0:
        current_len = len(latent_batch)
        if current_len < batch_size:
            pad_len = batch_size - current_len
            # print(f"[DEBUG] Padding last batch from {current_len} to {batch_size} to avoid JIT re-compile")
