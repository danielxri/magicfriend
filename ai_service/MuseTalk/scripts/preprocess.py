...
class AnalyzeFace:
    ...
    def __call__(self, im: np.ndarray) -> Tuple[List[np.ndarray], np.ndarray]:
        try:
            ...
            # Using FaceAlignment for BBOX (Existing logic)
            # bbox_list is a list of lists of detections. Batch size 1 -> bbox_list[0] is detections for image.
            bbox_batch = self.facedet.get_detections_for_batch(np.asarray(im))
            
            # Using FaceAlignment for LANDMARKS
            # calls get_landmarks on the single image in the batch
            # We pass detected_faces to speed it up / guide it.
            # bbox_batch[0] is the list of detections for this image.
            # If no detections, get_landmarks might search again or fail? 
            # If bbox_batch[0] is empty, we handle it.
            
            landmark_results = []
            if bbox_batch and len(bbox_batch) > 0 and bbox_batch[0] is not None:
                # get_landmarks returns list of arrays (one per face)
                # input image: remove batch dim -> im[0]
                landmark_results = self.facedet.get_landmarks(np.asarray(im)[0])
            
            # For compatibility with original return format:
            # We want 'face_land_mark' to be the landmarks of the MAIN face (first one).
    ...
    for vid in tqdm(vid_list, desc="Processing videos"):
        ...
        if vid.endswith('.mp4'):
            ...
            for frame_idx in range(total_frames):
                ...
                if frame_idx==0:
                    video_height,video_width,_ = frame_bgr_raw.shape
                
                # Original code logic: frame (Decord RGB) -> BGR2RGB -> frame_bgr (Result is BGR)
                # OpenCV reads BGR directly.
                frame_bgr = frame_bgr_raw
                # FaceAlignment expects RGB.
                frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
                
                if frame_idx == 0:
                    print(f"Frame 0 stats: Shape={frame_rgb.shape}, Mean={frame_rgb.mean()}")

                pts_list, bbox_list = analyze_face(frame_rgb)
                
                if frame_idx == 0:
                     print(f"BBox list: {bbox_list}")
                     print(f"Pts list: {pts_list}")
